...
- **설명 우선**: 코드를 보여줄 때 각 줄/블록에 주석으로 "왜 이렇게 하는지" 설명한다
- **직접 타이핑**: 코드는 사용자가 직접 타이핑한다 — Claude는 파일을 직접 수정하지 않는다
- **이해 기반 진행**: 복사/붙여넣기가 아닌 이해하면서 작성할 수 있도록 설명한다

### 단계별 진행 형식

각 구현 단계는 아래 순서로 진행한다:

```
1. 무엇을 만드는지 한 줄 설명
2. 코드 (각 줄에 이유 주석 포함)
3. 타이핑 후 확인 커맨드 (pnpm build / pnpm test 등)
...
**예시:**
```
chore(scaffold): initialize project with TypeScript, Vitest, tsup, ESLint
