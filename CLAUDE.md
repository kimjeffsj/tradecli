...
TradeLab은 SMC(Smart Money Concepts) 기반 전략 연구 + 자동매매 플랫폼이다.
**멀티 언어 아키텍처**: TypeScript(분석/전략/시그널) + Python(MT5 실행/텔레그램/데이터 분석).

**핵심 철학:**
- CLI는 인터페이스일 뿐 — Core Engine은 CLI 없이도 동작해야 한다
- 데이터 → 전략 → 백테스트 → 실행은 완전 분리
- 데이터 소스/실행 엔진/알림 채널은 어댑터 패턴으로 교체 가능
- TypeScript는 분석/전략, Python은 실행/연동/분석 — 역할을 명확히 분리

---

## Tech Stack

### TypeScript (Phase 1-4: src/)
...
### Python (Phase 5-7: executor/, analysis/)

| 영역 | 기술 |
|------|------|
| 언어 | Python 3.12+ |
| 웹 프레임워크 | FastAPI |
| MT5 | MetaTrader5 공식 패키지 |
| 테스트 | pytest |
| 패키지 매니저 | uv |
| 타입 체크 | mypy (strict) |
| 린팅 | ruff |
...
- **TS ↔ Python 통신은 REST API** (POST /api/signals)
- `any` 금지 — `unknown` + 타입 가드 사용 (TS)
...
├── index.ts                  # CLI 진입점
├── commands/                 # Oclif 커맨드 (analyze, backtest, scan, live)
...
├── src/executor/             # FastAPI + MT5 + Telegram
└── tests/
...
| `prd-v2.md` | 확장 기획서 (Phase 4-6 원본, TS 인터페이스 참고용) |
| `prd-v3.md` | 멀티 언어 아키텍처 기획서 (Phase 4-7: TS + Python) |
| `erd.md` | 데이터 모델 |
